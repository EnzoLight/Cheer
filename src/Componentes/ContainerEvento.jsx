...
const ContainerEvento = ({ titulo, descricao, data_hora_inicio, data_hora_fim, local }) => {
    return (
        <div className='ContainerEvento' style={{borderRadius: '25px'}}>
            ...
            <div className="d-flex justify-content-between" style={{marginRight:'30px', marginLeft:'30px', paddingBottom: '30px', marginBottom: '30px', fontSize: '0.8rem', fontWeight: '600' }}>
                <span style={{marginRight: '5px'}}><i className="fas fa-calendar-alt me-1"></i> {data_hora_inicio}</span>
                <span><i className="fas fa-calendar-alt me-1"></i> {data_hora_fim}</span>
                <span style={{width: 'auto', marginLeft: '5px'}}><i className="fas fa-map-marker-alt me-1"></i> {local}</span>
            </div>
        </div>
    );
}
